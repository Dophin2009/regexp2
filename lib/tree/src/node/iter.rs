...
unsafe impl<'a, K, V> Send for MutMarkedNode<'a, K, V>
where
    K: Send,
    V: Send,
{
}
unsafe impl<'a, K, V> Sync for MutMarkedNode<'a, K, V>
where
    K: Sync,
    V: Sync,
{
    ...
    N: NodeRef,
{
    ...
    fn next(&mut self) -> Option<N::Item> {
        loop {
            ...
            match push {
                None => {
                    return self.nodes.pop_back().map(|mut node| {
                        self.size -= 1;
...
impl<N> DoubleEndedIterator for Iter<N>
where
    N: NodeRef,
{
    fn next_back(&mut self) -> Option<N::Item> {
        loop {
            ...
            match push {
                None => {
                    return self.nodes.pop_front().map(|mut node| {
                        self.size -= 1;
...
where
    N: NodeRef,
{
    pub fn new<C, Min: ?Sized, Max: ?Sized>(
        root: Option<N>,
        size: usize,
        cmp: &C,
        min: Bound<&Min>,
    ...
    where
        C: Compare<Min, N::Key> + Compare<Max, N::Key>,
    {
    ...
    N: NodeRef,
{
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        (self.0.nodes.len(), Some(self.0.size))
    }
}

#[cfg(feature = "range")]
impl<N> DoubleEndedIterator for Range<N>
where
    N: NodeRef,
{
    fn next_back(&mut self) -> Option<N::Item> {
